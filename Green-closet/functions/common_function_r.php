...
function get_user_order_details(){
    ...
    $get_details = "Select * from `customer` where email = '$email'";
    $result_query8 = mysqli_query($con, $get_details);
    while($row_query = mysqli_fetch_array($result_query8)){
        $user_id = $row_query['user id'];
        if(!isset($_GET['edit_account'])){
            if(!isset($_GET['my_orders'])){
                if(!isset($_GET['delete_account'])){   
                   $get_orders = ("Select * from `orders` where `user id` = $user_id and order_status = 'pending'"); 
                   $result_order_query = mysqli_query($con, $get_orders);
                   $row_count = mysqli_num_rows($result_order_query);
                   if($row_count > 0){
                    echo "<h3 class='text-center mt-5 mb-2'> You have <strong>$row_count</strong> pending orders!</h3>
                    <p class='text-center mt-5'><a class = 'text-dark' href='profile.php?my_orders'>Order Details</a></p>";
                }else{
                    echo "<h3 class='text-center mt-5 mb-2'><strong>No pending orders!!!</strong></h3>
                    <p class='text-center mt-5'><a class = 'text-dark' href='../index.php'>Explore Products!!</a></p>";
                }
            }
        }
...
function product_sort($sort = ''){
    ...
    while($row = mysqli_fetch_assoc($result_query)){
      ...
      $product_price = $row['product_price'];
      echo "<div class ='col-md-4 mb-2'>
<div class='card flex-fill h-100' style='height: 400px;'>
<img src='./admin_area/product_images/$product_image1' class='card-img-top' alt='$product_title
<div class='card-body'>
    <h5 class='card-name'>$product_title</h5>
    <h6 class='card-name'>৳$product_price </h6>
    <div class='card-footer d-flex justify-content-between'>
    <a href='index.php?add_to_cart=$product_id' class='btn btn-dark flex-fill m-1'>Add to Cart</a>
    <a href='product_details.php?product_id=$product_id' class='btn btn-secondary flex-fill m-1'>View More</a>
            </div>
          </div>
        </div>
      </div>";    
